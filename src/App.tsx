...
export default function App() {
  ...
  const [archiveSuffix, setArchiveSuffix] = useState('_v1');
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (isArchive) {
          ...
          const newVersionNumber = archiveSuffix.replace(/^_?v?/, '');
  ...
  const handleUploadSubmit = async (files: File[], isArchive: boolean, version: string) => {
    try {
      ...
      if (isArchive) {
        setArchiveSuffix(`_v${version}`);
      }
      
      // Process the upload/archive
      await handleFileUpload(files, isArchive); // Pass parameters directly
