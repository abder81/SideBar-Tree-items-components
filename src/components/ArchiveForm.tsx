...
export const ArchiveForm: React.FC<ArchiveFormProps> = ({ 
  ...
  // Refined version increment logic
  useEffect(() => {
    const getNextVersion = (fileName: string): string => {
      // Check if filename contains '_v' pattern
      const versionMatch = fileName.match(/_v([\d.]+)/i);
      
      if (!versionMatch) {
        // No version found, start with version 2
        return '2';
      }

      const currentVersionString = versionMatch[1];
      
      // Split version into parts and convert to numbers
      const versionParts = currentVersionString.split('.').map(part => {
        const num = parseInt(part, 10);
        return isNaN(num) ? 0 : num;
      });

      // Validate that we have valid version parts
      if (versionParts.length === 0 || versionParts.some(part => part < 0)) {
        return '2'; // Fallback to version 2 if invalid
      }

      // Increment the last number in the version sequence
    ...
    if (currentFileName) {
      const nextVersion = getNextVersion(currentFileName);
  ...
  const validateVersion = (version: string): boolean => {
    // Allow only numbers separated by dots (1, 1.1, 1.1.2, etc.)
    const versionRegex = /^\d+(\.\d+)*$/;
    if (!versionRegex.test(version)) return false;

    // Ensure all parts are valid positive numbers
  ...
  const getPreviewName = (): string => {
    if (!versionNumber.trim()) return currentFileName;
    
    const lastDotIndex = currentFileName.lastIndexOf('.');
    const extension = lastDotIndex !== -1 ? currentFileName.substring(lastDotIndex) : '';
    const nameWithoutExt = lastDotIndex !== -1 
      ? currentFileName.substring(0, lastDotIndex)
      : currentFileName;
    
    // Remove existing version pattern if it exists
    const baseName = nameWithoutExt.replace(/_v[\d.]+$/i, '');
    return `${baseName}_v${versionNumber.trim()}${extension}`;
  };
  ...
  const handleArchive = async () => {
    ...
    // Use the current version number, or auto-increment if empty
    let finalVersion = versionNumber.trim();
    if (!finalVersion) {
      // Auto-increment logic - same as in useEffect
      const versionMatch = currentFileName.match(/_v([\d.]+)/i);
      if (!versionMatch) {
        finalVersion = '2';
      } else {
        const currentVersionString = versionMatch[1];
        ...
        const newVersionParts = [...versionParts];
        newVersionParts[newVersionParts.length - 1]++;
        finalVersion = newVersionParts.join('.');
      }
    }
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="text-center">
                  ...
                  <p className="font-mono text-xs text-gray-500 max-w-[100px] truncate">{getPreviewName()}</p>
                </div>
              </div>
            </div>
          )}
